...
def downLoadAllPicturesInOnePage(url, picnum, title):
    local = 'G:\\20160916\\picdown\\'+ keyWord + '\\'+ title + '\\'
    os.makedirs(local, exist_ok=True)
    try:
        html = getPageHtml(url)
        picturesSrcList, picturesTypeList = resolvePicturesSrc(html)
        for picOrder in range(len(picturesSrcList)):
            picname = local + keyWord + str(picnum + picOrder) + picturesTypeList[picOrder]
            downpic(picturesSrcList[picOrder], local,picname)
    except Exception as ex:
        print(ex.__str__())

def downpic(url,local,picname):
...
def downAllpicToDiffDict(pagesList, titlelist):
    ...
    if pagesList:
        for pagenum in range(len(pagesList)):
            ...
            downLoadAllPicturesInOnePage(pagesList[pagenum], picnum * pagenum, titlelist[pagenum])

@driversDecorator
def driversGroup(pagesList,titlelist,urllist,driver):
...
def splitToGroup(allurl,groupSize):
    ...
    for block in range(int(urlNum/groupSize)):
        ...
        titlelist, urllist = driversGroup(pagesList,titlelist,urllist)
    if allurl:
        titlelist, urllist = driversGroup(allurl,titlelist,urllist)
    return titlelist, urllist
